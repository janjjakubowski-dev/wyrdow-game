...
const TOWNS = {
        ...
        dialogue() {
          ...
          if (ms.sawOddLamp && !ms.goldaOddLampTalk) {
            return { lines: [
              ...
              ">From under the attendance ledger she takes a worn copper stylus and folds your fingers around it.",
              "When you climb — and you will climb, you have that kind of face — the first stroke is the flame's. A yod, above. Small. The way a lamp looks from far off.",
            ], onClose: () => {
              ms.goldaOddLampTalk = true;
              miedznoStrokes().flame = true;
              try { addJournalEntry('lamp_odd'); addJournalEntry('stroke_flame'); } catch (e) {}
              try { saveGame(); } catch (e) {}
            }, };
          }
          return { lines: [
  ...
  miedzno: (() => {
    ...
    return {
      ...
      // First voice of the Copper Hill — examine beats + the climb
      examines: [
        // The climb — only once all three strokes are taught
        { x: 18, y: 4.4, radius: 1.6, prompt: '[E] Touch the eyelid',
          name: 'The Sleeper',
          requires: () => {
            const st = miedznoStrokes();
            return st.flame && st.breath && st.bread && !gameState.miedznoState.eyeOpened;
          },
          lines: [
            ">The clay is warm. Under your palm, very far down, something turns its attention upward.",
            ">You take out the copper stylus, and you write, the way three people taught you.",
            ">The flame's stroke, small, above — Golda's yod. A lamp seen from far off.",
            ">The breath's stroke, the long slow diagonal — Srulik's line, drawn at the pace of a sleeper's chest.",
            ">The bread's stroke, small, below — Hana's yod. The crumb that is torn first and given first.",
            ">Aleph. The letter closes. And the hill opens its eye.",
            ">It is not like being seen. It is like being COUNTED — by something that has never once lost count of anyone.",
            ">In the amber of the iris, seven names hang arranged like a recipe. You know, without being told, how they are to be taken: read reflected. Attended nightly. Answered the same, not fast.",
            ">The seventh name is not written yet. The eye holds your gaze a moment on that empty line — not a threat. A vacancy. ADMINISTRATIVE.",
            ">Then the lid comes down, slow, a sleeper turning over. And the tear-streak, for the first time in eleven years, is dry.",
          ],
          action(scene) {
            scene.openDialogue('The Sleeper', this.lines, () => {
              ...
              // The hill exhales: one slow verdigris bloom over the eye
              try {
                const iso = cartToIso(18, 4.4);
                const wx = iso.x + scene.worldOffset.x, wy = iso.y + scene.worldOffset.y - 30;
                const flare = scene.add.graphics().setDepth(997);
                const proxy = { t: 0 };
                scene.tweens.add({
                  targets: proxy, t: 1, duration: 4200, ease: 'Sine.easeOut',
                  onUpdate: () => {
                    flare.clear();
                    flare.fillStyle(0x5ec0a4, 0.4 * (1 - proxy.t));
                    flare.fillCircle(wx, wy, 14 + proxy.t * 90);
                    flare.fillStyle(0xffd890, 0.5 * (1 - proxy.t));
                    flare.fillEllipse(wx, wy, 30 * (1 - proxy.t * 0.4), 12 * (1 - proxy.t * 0.4));
                  },
                  onComplete: () => flare.destroy(),
                });
                scene.playKnockThump && scene.playKnockThump(true);
              } catch (e) {}
            }, 'ornate');
          } },
        // After — the world remembers what happened here
        { x: 18, y: 4.4, radius: 1.6, prompt: '[E] The eyelid',
          name: 'The Sleeper',
          requires: () => gameState.miedznoState.eyeOpened,
          lines: [
            ">Closed again — but differently. The way a door is shut when someone means to come back through it.",
            ">The clay under your palm is warm, and the warmth follows your hand.",
          ] },
        ...
        { x: 12.8, y: 17.8, radius: 1.2, prompt: '[E] The recipe shelf',
          name: 'The Recipe Shelf',
          requires: () => gameState.miedznoState.met.hana,
          lines: [
            ">Above the kiln's mouth: papers in a hand that refuses to resolve — until you catch them in the polished flat of the bread peel.",
            ">Reflected, they are recipes. Plain ones. Rye, water, salt, a stone of the oven's own. And under each, a name, and under each name: 'first loaf down.'",
            ">Written backwards, for a reader on the other side of the surface.",
          ],
          action(scene) {
            gameState.miedznoState.sawMirrorRecipes = true;
            try { saveGame(); } catch (e) {}
            scene.openDialogue('The Recipe Shelf', this.lines, null, 'simple');
          } },
        ...
        return [
          {
            ...
            dialogue() {
              const st = miedznoStrokes();
              if (met('srulik') && (gameState.miedznoState.knocksAnswered || []).length > 0 && !st.breath) {
                return { lines: [
                  ">He is waiting for you outside the study house, ledger shut. He has heard.",
                  "You answered it. At the rail, at the stone — it doesn't matter where. It knocked, and you knocked back the same, and now I am done pretending this is a hill.",
                  ">He sits down on the study house step, suddenly an old man.",
                  "The town built him. Before the Collapse. Every family gave copper; every family gave a name to be written into the clay. My grandfather held the pen. It was never meant to mine — it was meant to CARRY us out, all of us, the day the seam failed.",
                  "The seam failed early. He was not finished waking. So he lay down over it — over the whole mine, sixty men still under — and he held. He is still holding. Eleven years, and the Regency calls it geology.",
                  ">His inked fingers make one slow stroke in the air: a long diagonal, drawn at the pace of a sleeper's chest.",
                  "The second stroke is breath. When you write it, write it SLOW. He has earned a slow hand.",
                ], onClose: () => {
                  st.breath = true;
                  try { addJournalEntry('stroke_breath'); } catch (e) {}
                  try { saveGame(); } catch (e) {}
                }, };
              }
              if (!met('srulik')) {
          ...
          },
          {
            ...
            dialogue() {
              const st = miedznoStrokes();
              if (met('hana') && gameState.miedznoState.sawMirrorRecipes && !st.bread) {
                return { lines: [
                  "You read the shelf. Good. Most people look at the peel, see their own face, and stop there. Faces are the least interesting thing a mirror does.",
                  "Backwards isn't hiding. Backwards is how you write for someone who reads from the other side. My grandmother baked for the mine. Every first loaf went down. Someone down there learned her hand — reflected.",
                  ">She dusts the flour from her fingers and takes your wrist, turning your hand palm up, businesslike.",
                  "The third stroke is bread's. A yod, below. Small, like a crumb, because the crumb is what's torn first and given first.",
                  "Now you have all the hands you need. Go up before you talk yourself out of it.",
                ], onClose: () => {
                  st.bread = true;
                  try { addJournalEntry('stroke_bread'); } catch (e) {}
                  try { saveGame(); } catch (e) {}
                }, };
              }
              if (!met('hana')) {
...
const gameState = {
  ...
  miedznoState: {
    ...
    strokes: { flame: false, breath: false, bread: false }, // the aleph
  },
...
// saves from before the strokes existed repair themselves on touch.
function miedznoStrokes() {
  const ms = gameState.miedznoState;
  if (!ms.strokes) ms.strokes = { flame: false, breath: false, bread: false };
  return ms.strokes;
}
...
Object.assign(JOURNAL_ENTRIES, {
  ...
  stroke_breath: {
    title: 'The Second Stroke: Breath', type: 'npc', category: 'Conversation', town: 'miedzno',
    text: "Srulik stopped pretending it is a hill. The town BUILT him, before the Collapse — every family gave copper and a name, and Srulik's grandfather held the pen. He was made to carry the town out; the seam failed before he finished waking, so he lay down over the mine and held. Eleven years. The second stroke is the long diagonal, drawn slow. He has earned a slow hand.",
  },
  stroke_bread: {
    title: 'The Third Stroke: Bread', type: 'npc', category: 'Conversation', town: 'miedzno',
    text: "Hana's grandmother baked for the mine, and someone down there learned her hand reflected — that is why the recipes are written backwards. The third stroke is a yod, below, small like a crumb, because the crumb is torn first and given first. I have all three strokes now. The hill is waiting.",
  },
  sleeper_eye: {
    title: 'The Eye Opened', type: 'observe', category: 'Discovery', town: 'miedzno',
    text: "I wrote the aleph on the eyelid in three strokes — flame, breath, bread — and the hill opened its eye. It was not like being seen. It was like being counted, by something that has never lost count of anyone. Then the lid came down slow, a sleeper turning over, and the tear-streak ran dry for the first time in eleven years.",
  },
  fragment_two: {
    title: 'Cure Codex — The Second Fragment', type: 'quest', category: 'The Cure', town: 'miedzno',
    text: "Seven names, arranged like a recipe, held in the amber of the Sleeper's iris. Not ingredients — attendances. To be read reflected, the way Hana reads. Attended nightly, the way Golda attends. Answered the same, not fast, the way the hill answers. The seventh line is empty — VACANT, you might say. ADMINISTRATIVE. The Cure is not a substance. It is a way of keeping people lit.",
  },
  ladder_room: {
